...
void resetGenoObj()
{
  for (int i = 0; i < geno.genoVecofPointers.size(); i++) {
    if (geno.genoVecofPointers[i] != NULL) {
      (*geno.genoVecofPointers[i]).clear();
      delete geno.genoVecofPointers[i];
      geno.genoVecofPointers[i] = NULL;
    }
  }
  geno.genoVecofPointers.clear();

  for (int i = 0; i < geno.genoVecofPointers_forVarRatio.size(); i++) {
    if (geno.genoVecofPointers_forVarRatio[i] != NULL) {
      (*geno.genoVecofPointers_forVarRatio[i]).clear();
      delete geno.genoVecofPointers_forVarRatio[i];
      geno.genoVecofPointers_forVarRatio[i] = NULL;
    }
  }
  geno.genoVecofPointers_forVarRatio.clear();

  geno.invstdvVec0.clear();
  geno.invstdvVec.clear();
  geno.invstdvVec0_forVarRatio.clear();
  geno.invstdvVec_forVarRatio.clear();

  geno.alleleFreqVec0.clear();
  geno.alleleFreqVec.clear();
  geno.alleleFreqVec0_forVarRatio.clear();
  geno.alleleFreqVec_forVarRatio.clear();

  geno.MACVec0.clear();
  geno.MACVec.clear();
  geno.MACVec0_forVarRatio.clear();
  geno.MACVec_forVarRatio.clear();
  geno.markerIndexVec0_forVarRatio.clear();
  geno.markerIndexVec_forVarRatio.clear();

  geno.ptrsubSampleInGeno.clear();
  geno.indicatorGenoSamplesWithPheno_in.clear();
  geno.MarkerswithMAFge_minMAFtoConstructGRM_indVec.clear();
  geno.kinValueVecFinal.clear();
  geno.stdGenoforSamples.clear();

  geno.m_OneSNP_Geno.clear();
  geno.m_OneSNP_StdGeno.clear();
  geno.m_DiagStd.clear();
  geno.m_DiagStd_LOCO.clear();
  geno.mtx_DiagStd_LOCO.clear();
  geno.subMarkerIndex.clear();
  geno.g_randMarkerIndforVR.clear();
  geno.xout.clear();
  geno.yout.clear();
  geno.startIndexVec.clear();
  geno.endIndexVec.clear();
  geno.startIndexVec_forvr.clear();
  geno.endIndexVec_forvr.clear();
  geno.indiceVec.clear();

  geno.numMarkersofEachArray = 0;
  geno.numofGenoArray = 0;
  geno.numMarkersofLastArray = 0;
  geno.numberofMarkers_varRatio = 0;
  geno.numberofMarkers_varRatio_common = 0;
  geno.numberofMarkerswithMAFge_minMAFtoConstructGRM = 0;
  geno.M = 0;
  geno.N = 0;
  geno.Nnomissing = 0;
  geno.Msub = 0;
  geno.Msub_MAFge_minMAFtoConstructGRM = 0;
  geno.Msub_MAFge_minMAFtoConstructGRM_singleChr = 0;
  geno.chromIndex = 0;
  geno.startIndex = 0;
  geno.endIndex = 0;
}
